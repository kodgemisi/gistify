...
function isAceAvailable() {
  return typeof window.ace == 'object' && typeof window.ace.EditSession == 'function';
};
...
//            Promises
// ================================
window.aceLibraryLoadingPromise = $.Deferred();
window.aceModeListLoadingPromise = $.Deferred();
var aceLoadingPromise = $.Deferred();

aceModeListLoadingPromise.done(function () {
  modelist = ace.require('ace/ext/modelist');//https://github.com/ajaxorg/ace/pull/1348
  aceLoadingPromise.resolve();

  delete window.aceLibraryLoadingPromise;
  delete window.aceModeListLoadingPromise;
});

// Load ace if not defined
if(!isAceAvailable()){
  // if ace lib is not defined replace the promise with
  // jqXHR which implements the Promise interface
  window.aceLibraryLoadingPromise = $.ajax({
    type: "GET",
    url: ACE_LIB_URL,
    dataType: "script",
    cache: true,
    success: function() {
      // aceLibraryLoadingPromise.resolve();
    },
    ...
    if(this.config.mode != 'create') {
      $.ajax({
        ...
        success: function(data) {
          ...
          aceLoadingPromise.done(function () {
            gist.render.call(gist);
          });
        },
        error: function(jqXHR,textStatus, errorThrown) {
          if(jqXHR.status == 404) {
